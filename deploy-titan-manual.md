# 🚀 Manual Titan Model Deployment

Since AWS CLI isn't available, here are the exact manual steps to deploy the Amazon Titan model code:
...
3. Find and click on function: **`recipe-generation-dev`**

### Step 2: Update the Bedrock Service Code
1. In the Lambda function, scroll down to the **Code source** section
2. In the file explorer on the left, navigate to: **`utils/bedrock-service.js`**
3. Click on the file to open it in the editor
4. **Select all content** (Ctrl+A) and **delete it**
5. **Copy the entire content** from the file `bedrock-service-titan.js` (see below)
6. **Paste it** into the Lambda editor
7. Click **"Deploy"** button (orange button at the top)
...
1. Wait for the deployment to complete (you'll see "Changes deployed" message)
2. Open your terminal/command prompt
3. Run: `node test-titan-deployment.js`

## Expected Results

✅ **Before**: "Model use case details have not been submitted"  
✅ **After**: Recipe generated successfully with Amazon Titan!

## If You Get Permission Errors

If you see "AccessDenied" errors, you need to update IAM permissions:

1. Go to: https://console.aws.amazon.com/cloudformation/
2. Find stack: **`recipe-generation-stack-dev`**
3. Click **"Update"**
4. Choose **"Replace current template"**
5. Upload the file: `ai-recipe-generator-backend/cloudformation/recipe-generation-stack.yaml`
6. Click through to complete the update
...
Once deployed, test the full application at:
http://ai-recipe-generator-web-app-914877613.s3-website-us-east-1.amazonaws.com

## Success Indicators

✅ No more "use case details" errors  
